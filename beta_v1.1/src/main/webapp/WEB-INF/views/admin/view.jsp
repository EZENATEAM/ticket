		...
		<c:if test='${not empty raw}'>
			...
			<c:choose>
				...
				<c:when test='${not empty festival}'>
					...
					<a href="${path}/admin/fes_mody?idx=${festival.idx}" class="btn btn-outline-primary btn-sm">수정하기</a>
				</c:when>
				<c:otherwise>
...
<%-- 				<li><a href="${path}/admin/festival_a?idx=${raw.idx}">${ raw.title }</a></li> --%>
				...
				<c:if test='${not empty raw.status}'>
					<li>status :
						<c:choose>
							<c:when test='${raw.status eq "N"}'><button type="button" class="btn btn-secondary btn-sm" disabled>미등록</button></c:when>
							<c:when test='${raw.status eq "A"}'><button type="button" class="btn btn-success btn-sm" disabled>등록됨</button></c:when>
							<c:when test='${raw.status eq "E"}'><button type="button" class="btn btn-danger btn-sm" disabled>종료됨</button></c:when>
				...
				<c:if test='${not empty raw.url}'><li>url : <a href='${ raw.url}&fromDt=20200101' target='_blank'>${ raw.url}</a></li></c:if>
				...
				<c:if test='${not empty raw.sourceTitle}'>
					...
					<c:choose>
						...
						<c:otherwise><a href="${ raw.sourceTitle }" target='_blank'>${ raw.sourceTitle }</a></c:otherwise>
					</c:choose></li>
				</c:if>
			</ul>
			<hr>
...
<%-- 		<c:if test='${raw.status eq "A" and not empty festival}'> --%>
		<c:if test='${not empty festival}'>
			...
			<ul>
				...
				<c:if test='${not empty festival.status}'>
					<li>status :
						<c:choose>
							<c:when test='${empty festival}'><button type="button" class="btn btn-secondary btn-sm" disabled>미입력</button></c:when>
							<c:when test='${not empty festival}'><button type="button" class="btn btn-success btn-sm" disabled>입력됨</button></c:when>
		...
		<c:if test='${not empty festival and empty events}'>
			...
			<form method="post" action="${path}/admin/evnt_post">
				...
				eDate : <input type="date"	name="eventList[0].eDate" min="${festival.startDate}" value="${festival.startDate}" /><br>
				eTime : <input type="time"	name="eventList[0].eTime" min="09:00" value="09:00" /><br>
				eTerm : <input type="number" name="eventList[0].eTerm" min=0/><br>
				...
				fIdx : <input type="text"	name="eventList[0].fIdx" value="${festival.rIdx}" readonly="true" /><br>
		...
		<c:if test="${not empty events}">
			...
			<c:forEach var="event" items="${events}">
				<ul>
					...
					<c:if test='${not empty event.fIdx}'><li>fIdx : ${event.fIdx}</li></c:if>
					<c:if test='${not empty event.aIdx}'><li>aIdx : ${event.aIdx}</li></c:if>
				</ul>
				<hr>
			</c:forEach>
		</c:if>
		<details>
			<summary>행사 등록하기</summary>
				<form method="post" action="${path}/admin/evnt_post">
				idx : <input type="text"	name="eventList[0].idx" /><br>
